...
logger = set_up_logger("callbacks.anime")

# Create a config object for Anime-specific settings
ANIME_CONFIG = {
    "tag_with_username": settings.anime_tag_with_username,
    "forced_tags": settings.anime_forced_tags,
    "allow_user_to_select_tags": settings.anime_allow_user_to_select_tags,
    "user_selectable_tags": settings.anime_user_selectable_tags,
    "add_monitored": settings.anime_add_monitored,
    "search_on_add": settings.anime_search_on_add,
    "season_monitor_prompt": settings.anime_season_monitor_prompt
}


async def handle_anime_callback(update, context, bot, convo, cid, i, op, op_flags):
    """Handle callbacks related to Anime (series).
    ...
    # Handle operations
    if op == "add":
        await handle_add_anime(update, context, bot, convo, cid, i, op_flags)
...
async def handle_add_anime(update, context, bot, convo, cid, i, op_flags):
    """Handle adding a series to Anime.
    ...
    if op_flags:
        ...
        if op_flags.get("tt") or op_flags.get("td"):
            handled = await handle_tag_selection(update, context, service, ANIME_CONFIG, convo, cid, i, op_flags)
            if handled:
                return
    ...
    if (
        ANIME_CONFIG["season_monitor_prompt"]
    ...
    if ANIME_CONFIG["allow_user_to_select_tags"] and not additional_data.get("td"):
        all_tags = service.get_filtered_tags(
            ANIME_CONFIG["user_selectable_tags"],
            ANIME_CONFIG["forced_tags"],
        )

        if not all_tags:
            logger.warning(
                "User tagging is enabled, but no tags found. Make sure there are tags in Anime matching your Searcharr configuration."
            )
    ...
    await process_tags(service, ANIME_CONFIG, cid, query.from_user)
    ...
    try:
        added = service.add_series(
            series_info=r,
            monitored=ANIME_CONFIG["add_monitored"],
            search=ANIME_CONFIG["search_on_add"],
            additional_data=get_add_data(cid),
        )
